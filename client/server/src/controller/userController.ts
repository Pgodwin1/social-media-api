...
export const Register = async (req: Request | any, res: Response) => {
    try {
        ...
        const userExist = await UserInstance.findOne({ where: { email: req.body.email }});
...
export const login = async (req: Request, res: Response) => {
    try {
        ...
        const userExist = await UserInstance.findOne({ 
            where: { email: email }
...
export const updateInfo = async (req: Request | any, res: Response) => {
    try {
      ...
      const info = await UserInstance.findOne({ where: { id: userId } });
      if (!info)
